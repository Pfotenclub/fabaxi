...
class TempVoiceBackend(Database):
    ...
    async def get_channel_id(self, owner_id):
        async with self.get_session() as session:
            record = await session.execute(
                select(TempVoiceTable.channel_id).where(TempVoiceTable.owner_id == owner_id))
            return record.scalars().first()
    async def change_channel_owner_id(self, channel_id, new_owner_id):
        async with self.get_session() as session:
            update_stmt = update(TempVoiceTable).where(TempVoiceTable.channel_id == channel_id).values(owner_id=new_owner_id)
